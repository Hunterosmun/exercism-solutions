...
defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, time_stamp} <- enter_building(now),
         ...
         {:ok, checksum} <- stamp_form(time_stamp, counter_number, form),
         number <- get_new_passport_number(time_stamp, counter_number, checksum) do
      {:ok, number}
    else
      {:error, message} -> {:error, message}
      {:coffee_break, _} -> {:retry, NaiveDateTime.add(now, 900, :seconds)}
    end
  end
